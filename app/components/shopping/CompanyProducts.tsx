        ...
        <div className=" grid grid-cols-2 gap-3 md:grid-cols-4 lg:gap-x-4 xl:gap-x-6 justify-between md:gap-y-6 lg:gap-y-16 ">
          { isLoading ? <Skeleton height={70} /> :
               items.slice(0,16).map((item) => (
                <ProductCard key={item._id} slug={item.slug}  _id={item._id} name={item.name} image={item.image}  stars={item.stars} price={item.currentPrice} previousPrice={item.previousPrice} liked={item.liked}  /> 
            ) )
          }
        </div> 
    </div>
