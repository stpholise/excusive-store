...
const slides: Slide[] = [
  {
    ...
    image: "/image/hero_endframe__cvklg0xk3w6e_large 2.jpg",
  ...
  {
    logo: "/icons/apple-logo.svg",
    image: "/image/about-side-image.svg",
    title: "iPhone 14 Series",
    link: "/about",
    text: "Up to 10% off Voucher",
    color: "bg-[#EB7EA8]",
...
const Hero = () => {
  ...
  return (
    <div
      className="embla  overflow-hidden relative w-full flex h-48 min-w-80 items-center justify-center
        ...
        xl:min-w-[804px] xl:max-w-[894px]   xl:w-[904px]  "
      ref={emblaRef}
    >
      <div
        className=" flex flex-nowrap px-8 h-40  gap-3 w-full min-w-80
            xs:w-[400px] xs:min-w-[400px] xs:max-w-[400px]
      ...
      >
        {slides.map((slide, index) => (
          <div
            key={index}
            className={`embla__slide   rounded-lg w-full min-w-72 
          ...
          >
            <div className="flex flex-col  md:w-64 lg:w-[444px]  text-gray-100 lg:h-62 xl:h-72  gap-2  md:gap-4">
              <div className="flex gap-1 md:gap-2 items-center">
                ...
                />
                <h3 className=" text-xs md:text-sm">{slide.title}</h3>
              </div>
              <h2 className="md:text-2xl text-xl lg:text-4xl xl:text-5xl font-semibold leading-snug">
                {" "}
                {slide.text}
              </h2>
