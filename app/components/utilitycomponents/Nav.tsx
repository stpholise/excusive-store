  ...
  return (
    <div className={clsx("dark:invert",{
        ...
        <ul className={clsx('lg:flex   flex-row lg:flex-row gap-4 text-black ', {
            'flex-col uppercase md:capetalize font-semibold translate-x-1 lg:flex-nowrap text-gray-800 gap-6 text-lg' : showMenu,
            'hidden' : !showMenu
        })}>
            {navItems.map((item, index) => (
                <li key={`${item.href}${index}`} className={clsx('', {
                    'border-b-2 border-gray-400 py-4 lg:py-0 lg:border-b-0': showMenu,
                    // 'border-b-0': showMenu && index === navItems.length-1
                })}>
