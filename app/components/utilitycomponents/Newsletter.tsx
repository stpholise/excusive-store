...
const Newsletter = () => {
  ...
  const handleNewsLetterSubmission = async (
    values: FormValues,
    { setStatus, setSubmitting, resetForm }: FormikHelpers<FormValues>,
  ) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.kit.com/v4/forms/8faacd7482/subscribe",
        {
          method: "POST",
          headers: {
          ...
          <Form>
            <div className="w-full border-2 border-gray-300 flex gap-1 rounded-md my-2 p-2">
              <Field
                ...
                className="outline-none w-10/12 bg-black font-semibold text-white "
              />
              <button
                title="send"
                type="submit"
